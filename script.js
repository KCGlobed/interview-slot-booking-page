...
(function () {
    ...
    bookSlotBtn.addEventListener('click', function () {
        ...
        var baseUrl = window.GCC_Base_url || "https://gcc-backend-dev.gccschool.com";
    ...
    function handle(formId, successId) {
        ...
        form.addEventListener('submit', function (e) {
            ...
            var baseUrl = window.GCC_Base_url || VITE_API_BASE_URL;
...
(function initStudentVideosSlider() {
    ...
    const prevBtn = document.getElementById('student-videos-prev');
    const nextBtn = document.getElementById('student-videos-next');
    if (!grid) return;
...
/* ════════════════ CAMPUS PHOTOS SLIDER LOGIC ════════════════ */
(function initCampusSlider() {
    const strip = document.getElementById('campus-strip') || document.querySelector('.campus-strip');
    const prevBtn = document.getElementById('campus-prev');
    const nextBtn = document.getElementById('campus-next');
    if (!strip) return;

    function getScrollStep() {
        const firstCard = strip.querySelector('.ph, .campus-card');
        if (firstCard) {
            const gap = 16;
            const cardsToScroll = window.innerWidth < 768 ? 1 : 2;
            return (firstCard.offsetWidth + gap) * cardsToScroll;
    ...
    function updateControls() {
        ...
        const atEnd = strip.scrollLeft >= maxScroll - 6;
        if (prevBtn) { prevBtn.disabled = atStart; prevBtn.setAttribute('aria-disabled', String(atStart)); }
        if (nextBtn) { nextBtn.disabled = atEnd; nextBtn.setAttribute('aria-disabled', String(atEnd)); }
    }
    ...
    strip.addEventListener('wheel', function (e) {
        if (e.deltaY !== 0 && strip.scrollWidth > strip.clientWidth) {
            const maxScroll = strip.scrollWidth - strip.clientWidth;
            const canScrollLeft = strip.scrollLeft > 0 && e.deltaY < 0;
            const canScrollRight = strip.scrollLeft < (maxScroll - 1) && e.deltaY > 0;
            if (canScrollLeft || canScrollRight) {
                e.preventDefault();
                strip.scrollBy({ left: e.deltaY * 1.2, behavior: 'smooth' });
            }
        }
    }, { passive: false });
})();
